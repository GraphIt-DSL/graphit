...
void swarm_main() {
        ...
        while ((update[16]) != (0)) {
                update[16] = 0;
		swarm_runtime::Stride<int> stride(edges.num_edges); // MANUALLY ADDED
		for (int i = 0, m = stride.largerRange; i < m; i++) {  // MANUALLY ADDED
                        int _iter = stride(i);  // MANUALLY ADDED
			if (_iter < stride.smallerRange) {  // MANUALLY ADDED
