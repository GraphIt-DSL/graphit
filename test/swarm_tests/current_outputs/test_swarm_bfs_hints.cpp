...
void swarm_main() {
	...
	//inFrontier = new bool[swarm_runtime::builtin_getVertices(edges)]();
	...
	swarm_frontier.for_each_prio([](unsigned level, int src, auto push) {
		...
		//SCC_OPT_LOOP_COARSEN_FACTOR(SWARM_CACHE_LINE/sizeof(int))
		for (int i = edgeZero; i < edgeLast; i++) {
			//auto dst_addr = &(edges.h_edge_dst[i]);
			int dst = edges.h_edge_dst[i];
			auto* parent_addr = &parent[dst];
			//auto* parent_copy = parent;
			SCC_OPT_TASK();
			//SCC_OPT_CACHELINEHINT(&parent_copy[dst]);
			SCC_OPT_CACHELINEHINT(parent_addr);
			if (toFilter(dst)) {
				{
					parent[dst] = src;
					push(level + 1, dst);
