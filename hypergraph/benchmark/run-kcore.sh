...
for suff in "" "-Sparse" "-EdgeAware" "-Sparse-EdgeAware"; do
...
echo "graphit start"
for sched in $(ls ../../hypergraph/schedules/kcore/*.gt); do
    echo $sched
    ...
    for n in {1,2,4,8,16,32,48}; do
	echo "threads=$n"
	CILK_NWORKERS=$n "./$gp" "$inp/$graph.sg" ${graphinfo[1]} ${graphinfo[3]}
    done
    echo "threads=99"
        numactl -i all "./$gp" "$inp/$graph.sg" ${graphinfo[1]} ${graphinfo[3]}
    echo "threads=999"
done
