...
make clean &> /dev/null 
graphinfo=($(head -5 "$winp/$graph" | tr "\n" "\n"))
echo "CILK"
echo $graph
head -5 "$winp/$graph"
...
for suff in "" "-Sparse" "-Dense" "-Threshold" "-EdgeAware" "-Sparse-EdgeAware" "-Dense-EdgeAware" "-Threshold-EdgeAware"; do
	...
	echo "threads=$n"
	CILK_NWORKERS=$n "$hp$suff" -rounds 3 "$winp/$graph"
    done
    echo "threads=99"
        numactl -i all "$hp$suff" -rounds 3 "$winp/$graph"
    echo "threads=999"
done
...
for sched in $(ls ../../hypergraph/schedules/same/*.gt); do
    ...
    for n in {1,2,4,8,16,32,48}; do
	echo "threads=$n"
	CILK_NWORKERS=$n "./$gp" "$winp/$graph.wsg" 0 ${graphinfo[1]} ${graphinfo[3]}
    done
    echo "threads=99"
        numactl -i all "./$gp" "$winp/$graph.wsg" 0 ${graphinfo[1]} ${graphinfo[3]}
    echo "threads=999"
done
