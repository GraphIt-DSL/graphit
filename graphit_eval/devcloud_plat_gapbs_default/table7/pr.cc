...
#include <vector>
#include <algorithm>
#include "./../../../src/runtime_lib/intrinsics.h"
...
int main(int argc, char * argv[])
{
  ...
  for ( int trail = (0) ; trail < (16) ; trail++ )
  {
    startTimer() ;
    ligra::parallel_for_lambda((int)0, (int)builtin_getVertices(edges) , [&] (int vertexsetapply_iter) {
    ...
    for ( int i = (0) ; i < (20) ; i++ )
    {
      ...
      if ((builtin_sum < double  > (error, builtin_getVertices(edges) ) ) < (epsilon))
       { 
        std::cout << "Trial Number"<< std::endl;
        std::cout << i<< std::endl;
        break;
       } 
    }
